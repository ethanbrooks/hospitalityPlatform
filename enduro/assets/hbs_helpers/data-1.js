...
const path = require('path')
    ...
    enduro.templating_engine.registerHelper('data-1', function (options) {
    ...
    return enduro.api.pagelist_generator.get_cms_list()
        ...
        .then(() => {
            ...
            for (let i = max_posts, l = blog_entries.length; i < l; i += max_posts) {
                let context = {
                    page_name: 'data-' + (i / max_posts + 1),
                    entries: blog_entries.slice(i, i + max_posts),
                    first: false,
                    last: blog_entries.length - i <= max_posts,
                    next_page: '/data-' + (i / max_posts + 2) + '/',
                    prev_page: i === max_posts? '/' : ('/data-' + (i / max_posts) + '/')
                }
                ...
                // drop `index.html` files in `data-N` folders so they can be served as static

                enduro.api.temper.render('data-1', context)
                .then(data => {
