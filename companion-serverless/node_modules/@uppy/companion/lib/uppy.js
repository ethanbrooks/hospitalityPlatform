...
module.exports.app = (options = {}) => {
    ...
    app.use((req, res, next) => {
        res.header('Access-Control-Allow-Headers', [res.get('Access-Control-Allow-Headers'), 'uppy-auth-token'].join(', '));
        next();
    });
    ...
    wss.on('connection', (ws) => {
        ...
        function sendProgress(data) {
            ws.send(jsonStringify(data), (err) => {
                if (err)
                    logger.error(err, 'socket.progress.error');
            });
        }
        ...
        if (redisClient) {
            redisClient.get(`${STORAGE_PREFIX}:${token}`, (err, data) => {
                if (err)
                    logger.error(err, 'socket.redis.error');
                if (data) {
