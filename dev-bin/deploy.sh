...
aws s3 sync www/ s3://${S3BUCKETNAME} --profile ${AWSPROFILE} --metadata-directive REPLACE --cache-control max-age=31536000,public --acl public-read --exclude "stats-es5.json" --exclude "stats-es2015.json" --exclude "index.html" --exclude "*.css" --exclude "*.js" || { echo 'ERROR: s3 sync failed' ; exit 1; }

# Sync js&css setting content-encoding explictly to gzip
#aws s3 sync www/ s3://${S3BUCKETNAME} --profile ${AWSPROFILE} --metadata-directive REPLACE --cache-control max-age=31536000,public --acl public-read --exclude "*" --include "*.css" --include "*.js" --content-encoding gzip || { echo 'ERROR: s3 js/css sync failed' ; exit 1; }
# copy index.html making sure its not cached at the edge
#aws s3 cp www/index.html s3://${S3BUCKETNAME}/index.html --profile ${AWSPROFILE} --metadata-directive REPLACE --cache-control max-age=0,no-cache,no-store,must-revalidate --content-type text/html --acl public-read --content-encoding gzip || { echo 'ERROR: s3 cp index failed' ; exit 1; }
